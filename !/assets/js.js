"use strict";var _get = function get(_x, _x2, _x3) { var _again = true; _function: while (_again) { var object = _x, property = _x2, receiver = _x3; desc = parent = getter = undefined; _again = false; if (object === null) object = Function.prototype; var desc = Object.getOwnPropertyDescriptor(object, property); if (desc === undefined) { var parent = Object.getPrototypeOf(object); if (parent === null) { return undefined; } else { _x = parent; _x2 = property; _x3 = receiver; _again = true; continue _function; } } else if ('value' in desc) { return desc.value; } else { var getter = desc.get; if (getter === undefined) { return undefined; } return getter.call(receiver); } } };var _createClass = (function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ('value' in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; })();function _inherits(subClass, superClass) { if (typeof superClass !== 'function' && superClass !== null) { throw new TypeError('Super expression must either be null or a function, not ' + typeof superClass); } subClass.prototype = Object.create(superClass && superClass.prototype, { constructor: { value: subClass, enumerable: false, writable: true, configurable: true } }); if (superClass) Object.setPrototypeOf ? Object.setPrototypeOf(subClass, superClass) : subClass.__proto__ = superClass; }function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError('Cannot call a class as a function'); } }var $window = $(window),    $document = $(document),    $html = $('html'),    $body = $('body'),    $body_preventMouseover = false,    _g = {	isinit: false,	isload: false,	isfocus: document.hasFocus ? document.hasFocus() : true,	everfocus: document.hasFocus ? document.hasFocus() : true,	posttime: [],	time_format: 'm月d日',	time_format2: 'Y年m月d日',	time_diff_range: 4 * 24 * 60 * 60 * 1000,	time_diff_range2: 365 * 24 * 60 * 60 * 1000,	last: {		width: 0,		height: 0	},	interval: {},	initSize: 20,	baseSize: 10,	baseMultiper: 1,	lastBaseSize: 10,	pixelRatio: window.devicePixelRatio ? window.devicePixelRatio : screen.deviceXDPI ? screen.deviceXDPI / 72 : 1,	maxMultiper: Math.max(1, Math.ceil((screen.availHeight || screen.height || $window.height()) / 800 * 10) / 10),	strings: {},	inputIndex: 0,	lang: 'zh_cn',	_: false},    _p = {	comp: {},	el: {}},    _frame = {	dom: {},	init_all: function init_all() {		for (var i in _frame) {			if (_frame[i].init) {				_frame[i].init();			}		}	}},    _module = {},    _page = {},    _data = {};if (typeof _huScrolled == 'undefined') var _huScrolled = {};if (!_huScrolled.ver || _huScrolled.ver < 1.2) {	_huScrolled.ver = 1.2;	_huScrolled.timeout = false;	_huScrolled.throttle = _huScrolled.throttle || 100;	var bIEnew = /\(Windows NT [0-9\.]+.+Trident\/[0-9\.]+.+rv.{1}[0-9\.]+\)/.test(navigator.userAgent),	    bIE = !!(window.attachEvent && !window.opera) || bIEnew;	var el = bIE && parseFloat(navigator.appVersion.split("MSIE")[1]) < 9 ? $window : $document;	el.on({		'scroll.huScrolled': function scrollHuScrolled() {			if (_huScrolled.timeout) return true;			_huScrolled.timeout = setTimeout(function () {				$document.trigger('huScrolled');				_huScrolled.timeout = null;			}, _huScrolled.throttle);		}	});}if (typeof _huResized == 'undefined') var _huResized = {};if (!_huResized.ver || _huResized.ver < 2.1) {	_huResized.ver = 2.1;	_huResized.throttle = _huResized.throttle || 300;	_huResized.startSize = {};	_huResized.viewport = function () {		var e = window,		    a = 'inner';		if (!('innerWidth' in window)) {			a = 'client';			e = document.documentElement || document.body;		}		return {			width: e[a + 'Width'],			height: e[a + 'Height']		};	};	$window.on({		'resize.huResized': function resizeHuResized(e, settings) {			settings = settings || {};			var viewport = _huResized.viewport(),			    sWidth = _huResized.startSize.width || viewport.width,			    sHeight = _huResized.startSize.height || viewport.height,			    toshow = Math.abs(viewport.width - sWidth) > 50 || Math.abs(viewport.height - sHeight) > 50;			if (_huResized.timeout) {				clearTimeout(_huResized.timeout);				_huResized.timeout = null;			}			if (!_huResized.topmask) _huResized.topmask = $('<div/>').css({				'z-index': '16777269',				'display': 'none',				'position': 'fixed',				'width': '100%',				'height': '100%',				'top': 0,				'left': 0,				'background-color': 'rgba(0,0,0,.5)'			}).appendTo($body);			if (toshow && !settings.isInitial && !_huResized.maskShowing) {				_huResized.topmask.css('display', 'block');				_huResized.maskShowing = true;			}			if (!toshow && !_huResized.isChanging) {				_huResized.startSize = {					width: viewport.width,					height: viewport.height				};			}			_huResized.isChanging = true;			_huResized.timeout = setTimeout(function () {				$window.trigger('huResized');				_huResized.timeout = null;				_huResized.maskShowing = false;				_huResized.isChanging = false;				_huResized.topmask.css('display', 'none');				viewport = _huResized.viewport();				_huResized.startSize = {					width: viewport.width,					height: viewport.height				};			}, settings.isInitial ? 0 : _huResized.throttle);		}	});}if (typeof _huCss == 'undefined') var _huCss = {};if (!_huCss.ver || _huCss.ver < 1.2) {	_huCss.ver = 1.2;	_huCss.cssprefix_result = {};	_huCss.csscheck = function (prop) {		if (!_huCss.csscheck_div) {			_huCss.csscheck_div = document.createElement("div");		}		if (prop in _huCss.csscheck_div.style) {			return true;		} else {			var strs = prop.split('-');			prop = strs[0];			for (var i = 1; i < strs.length; i++) {				prop += strs[1].substr(0, 1).toUpperCase() + strs[1].substr(1);			}			return prop in _huCss.csscheck_div.style;		}	};	_huCss.csscheck_full = function (prop) {		return _huCss.cssprefix(prop, null, true);	};	_huCss.cssprefix = function (prop, onlyPrefix, isCheck) {		if (_huCss.cssprefix_result[prop]) {			var b = _huCss.cssprefix_result[prop];		} else if (_huCss.cssprefix_result[prop] === false) {			if (isCheck) return false;			var b = '';		} else {			var b = '',			    pre = ['-webkit-', '-moz-', '-ms-', '-o-'],			    check = _huCss.csscheck(prop);			if (!check) {				b = false;				for (var i = 0; i < pre.length; i++) {					if (_huCss.csscheck(pre[i] + prop)) {						b = pre[i];						break;					}				}			}			_huCss.cssprefix_result[prop] = b;			if (isCheck) {				b = b === false ? false : true;				return b;			}		}		b = b === false ? '' : b;		return onlyPrefix ? b : b + prop;	};	_huCss.csscheck_3d = function () {		return _huCss.csscheck_full('perspective');	};	_huCss.createSheet = function (name) {		var sheet = document.createElement('style');		if (name) sheet.title = name;		document.getElementsByTagName('head')[0].appendChild(sheet);		if (!window.createPopup) {			sheet.appendChild(document.createTextNode(''));		}		if (name) {			for (var i = 0; i < document.styleSheets.length; i++) {				if (document.styleSheets[i].title == name) {					return document.styleSheets[i];				}			}		}		return document.styleSheets[document.styleSheets.length - 1];	};	_huCss.getSheet = function (sheet) {		sheet = sheet || _huCss.sheet;		if (!sheet) {			_huCss.sheet = _huCss.createSheet('__huCss_sheet');			sheet = _huCss.sheet;		}		return sheet;	};	_huCss.getCssRulesNum = function (sheet) {		sheet = _huCss.getSheet(sheet);		return sheet.cssRules ? sheet.cssRules.length : 0;	};	_huCss.addRule = function (selector, declaration, index, sheet) {		var v = '';		sheet = _huCss.getSheet(sheet);		for (var i in declaration) {			if (!_huCss.csscheck_full(i)) {				if (i == 'opacity' && _huCss.csscheck_full('filter')) {					v += 'filter:alpha(opacity=' + declaration[i] * 100 + ')';				}			} else {				v += _huCss.cssprefix(i) + ':' + declaration[i] + ';';			}		}		if (!index && index !== 0) {			index = sheet.cssRules ? sheet.cssRules.length : _huCss.getCssRulesNum();		}		if (sheet.insertRule) {			sheet.insertRule(selector + '{' + v + '}', index);		} else {			selector = selector.split(',');			for (i = 0; i < selector.length; i++) {				sheet.addRule(selector[i], v, index);			}		}	};	_huCss.removeRule = function (index, sheet) {		if (!index && index !== 0) return false;		sheet = _huCss.getSheet(sheet);		try {			sheet.deleteRule(index);		} catch (e) {			sheet.removeRule(index);		}	};}var _UA = navigator.userAgent,    bChrome = /Chrome/.test(_UA),    bChromeVer = bChrome ? /Chrome\/([0-9\.]+)/.exec(navigator.appVersion) : false,    bSafari = /Safari/.test(_UA) && !bChrome,    bFirefox = /Firefox/.test(_UA),    bOpera = /Opera/.test(_UA),    bWebkit = /WebKit/.test(_UA),    bWebkitVer = bWebkit ? /(AppleWebKit|Safari)\/([0-9\.]+)/.exec(navigator.appVersion) : false,    bIEnew = /\(Windows NT [0-9\.]+.+Trident\/[0-9\.]+.+rv.{1}[0-9\.]+\)/.test(_UA),    bIEnewVer = bIEnew ? parseFloat(_UA.split('rv:')[1]) : false,    bIE = !!(window.attachEvent && !window.opera) || bIEnew,    bIE6 = bIE && parseFloat(navigator.appVersion.split("MSIE")[1]) < 7,    bIE7 = bIE && parseFloat(navigator.appVersion.split("MSIE")[1]) < 8,    bIE8 = bIE && parseFloat(navigator.appVersion.split("MSIE")[1]) < 9,    bIE9 = bIE && parseFloat(navigator.appVersion.split("MSIE")[1]) < 10,    bIE10 = bIE && parseFloat(navigator.appVersion.split("MSIE")[1]) < 11,    bIE11 = bIEnewVer && bIEnewVer < 12,    bGecko = !bIE && !bIEnew && !bWebkit && _UA.indexOf("Gecko") != -1,    bIphone = /iPhone/i.test(_UA),    bIpad = /iPad/i.test(_UA),    bAndroid = /Android/i.test(_UA),    bIOS = false,    bIOSver = bIphone || bIpad ? /CPU.*OS\s*([0-9_]+)/i.exec(navigator.appVersion) : false,    bMobile = bIphone || bIpad || bAndroid,    bCSSrem = !bIE8,    bCSS3 = _huCss.csscheck_full('border-radius'),    bCSS3A = _huCss.csscheck_full('animation'),    bCSS3flex = _huCss.csscheck_full('flex'),    b3D = _huCss.csscheck_full('perspective'),    bCSS3calc = bCSSrem,    bTouch = /Touch/.test(_UA),    bUnsupport = bIE7,    bHTML5m3u8 = bMobile,    bAccessYoutube = false,    bAccessTwitter = false,    bAccessFacebook = false;if (bWebkitVer && bWebkitVer.length) bWebkitVer = bWebkitVer[2];if (bChromeVer && bChromeVer.length) bChromeVer = parseFloat(bChromeVer[1]);if (bIOSver && bIOSver.length) {	bIOSver = parseFloat(bIOSver[1].replace(/_/, '.'));	bIOS = true;}if (bChromeVer && bChromeVer < 29 || bIOSver && bIOSver < 7) bCSS3flex = false;if (bIOSver && bIOSver < 6) {	bCSS3calc = false;}if (bGecko) {	$html.addClass('gecko');} else if (bIE11 && !bIE10) {	$html.removeClass('ie9').addClass('ie11' + (bTouch ? ' ie-touch' : ''));} else if (bIE10 && !bIE9) {	$html.removeClass('ie9').addClass('ie10' + (bTouch ? ' ie-touch' : ''));} else if (bMobile) {	$html.addClass('mobile');	if (bIOS) {		$html.addClass('ios');	}	if (bIphone) {		$html.addClass('iphone');	}	if (bIpad) {		$html.addClass('ipad');	}	if (bAndroid) {		$html.addClass('android');	}} else if (bWebkitVer < 537) {	$html.addClass('oldwebkit');}if (bTouch) {	$html.addClass('touch');}if (navigator.userAgent.match(/IEMobile\/10\.0/)) {	var msViewportStyle = document.createElement("style");	msViewportStyle.appendChild(document.createTextNode("@-ms-viewport{width:auto!important}"));	document.getElementsByTagName("head")[0].appendChild(msViewportStyle);}_g.getUrl = function () {	return location.href.split('#')[0];};_g.uriSearchArr = {};_g.uriHashArr = {};_g.timeHash = function () {	var d = new Date();d = d.getTime();return d;};_g.uriSearch = function (name) {	if (!_g.uriSearchArr.length) {		var _s = location.search ? location.search.split('?')[1] : '';		_s = _s.split('&');		for (var i = 0; i < _s.length; i++) {			var h = _s[i].split('=');			_g.uriSearchArr[h[0]] = h[1] || '';		}	}	return !name ? location.search.substr(location.search.indexOf('?') + 1) : _g.uriSearchArr[name];};_g.uriHash = function (name, val, value) {	var curH = location.hash,	    _h = curH ? curH.split('#')[1] : '';	_h = _h.split('&');	if (!_g.uriHashInited) {		_g.uriHashArr = {};		for (var i in _h) {			var h = _h[i].split('=');			if (h[0] !== '') _g.uriHashArr[h[0]] = h[1] || false;		}		_g.uriHashInited = true;	}	if (typeof name == 'object') {		for (var k in name) {			curH = _g.uriHash(k, name[k], curH);		}		location.hash = curH;	} else {		if (val === false || val === '') {			curH = curH.replace('&' + name + '=' + _g.uriHashArr[name], '').replace(name + '=' + _g.uriHashArr[name], '');			if (curH == '#' || curH == '' || !curH) curH = '_';			location.hash = curH;		} else if (typeof val != 'undefined') {			if (val == _g.uriHashArr[name]) return val;			var _val = _g.uriHashArr[name] ? curH.replace(name + '=' + _g.uriHashArr[name], name + '=' + val) : curH + (curH == '' ? '#' : '&') + name + '=' + val;			_g.uriHashArr[name] = val;			if (value) {				return _val;			} else {				location.hash = _val;			}			return val;		}	}	return !name ? location.hash.substr(location.hash.indexOf('#') + 1) : _g.uriHashArr[name] || false;};_g.randNumber = function (maxn) {	var d = new Date();d = d.getTime();d = (d * 9301 + 49297) % 233280;if (!maxn) {		maxn = 1000;	};return Math.ceil(d / 233280.0 * maxn);};_g.randInt = function (max, min) {	return Math.floor(Math.random() * parseInt(max) + (min ? parseInt(min) : 0));};_g.scrollTo = function (tar, y) {	if (isNaN(tar)) {		tar = $(tar);		var cur = tar.scrollTop();	} else {		y = tar;		tar = $('html,body');		var cur = $window.scrollTop();	}	var diff = Math.abs(cur - y),	    height = $window.height(),	    time = diff <= height ? 200 : 200 * (diff / height) * (2 / 3);	tar.stop().animate({		'scrollTop': y	}, time);};_g.get_em = function (num, el, fix) {	var _num = parseFloat(num);	el = el || $body;	fix = fix || 0;	if (isNaN(_num)) return num;	return _num / parseFloat(el.css('font-size')) + fix + 'em';};_g.get_fontsize = function (el) {	el = el || $body;	return parseInt(el.css('font-size'));};_g.get_basesize = function () {	return parseInt($body.css('font-size')) / _g.initSize * 10;};_g.get_basesize_true = function () {	return parseInt($body.css('font-size'));};_g.get_basemultiper = function () {	return parseFloat(_g.get_basesize() / 10);};_g.get_rem = function (unit) {	var num = parseFloat(unit);	if (isNaN(num)) return unit;	if (!bCSSrem) return unit;	return num / _g.initSize + 'rem';};_g.rem = function (unit, demical) {	var num = parseFloat(unit);	if (isNaN(num)) return unit;	if (!bCSSrem) return unit;	num = num / (_g.get_basesize() / 10) / _g.initSize;	if (demical) return num + 'rem';	return Math.round(10 * num) / 10 + 'rem';};_g.px = function (unit, only_number) {	var num = parseFloat(unit);	if (isNaN(num)) return unit;	if (bIE8) return unit;	return num * _g.initSize + (only_number ? 0 : 'px');};_g.get_animate_duration = function (duration) {	return _g.isfocus ? duration : 0;};_g.goto_hash = function (hash, time) {	hash = hash || location.hash;	hash = hash.replace(/^([\#]{0,1})(.+)/, '$2');	if (hash[0] == '!') return false;	var tar = $('#' + hash);	if (!tar.length) return false;	var tarY = tar.offset().top,	    curY = $window.scrollTop(),	    diff = Math.abs(curY - tarY),	    height = $window.height();	time = time == null ? diff <= height ? 200 : 200 * (diff / height) * (2 / 3) : time;	if (time) {		$('html,body').stop().animate({			'scrollTop': tarY		}, time, function () {			location.hash = hash;		});	} else {		$('html,body').scrollTop(tarY);		location.hash = hash;	}	return hash;};_g.time = function (str) {	if (!str) return _g.timeNow();	var time,	    patt = [{		exp: /(\d{4}).(\d{1,2}).(\d{1,2})[^0-9]*(\d{0,2})[:]{0,1}(\d{0,2})[:]{0,1}(\d{0,2})([\+\-])(\d{2})\:(\d{2})/i,		p: {			year: 1,			month: 2,			day: 3,			hour: 4,			min: 5,			sec: 6,			zoneD: 7,			zoneH: 8,			zoneM: 9		}	}, {		exp: /(\d{1,2}).(\d{1,2}).(\d{4})[^0-9]*(\d{0,2})[:]{0,1}(\d{0,2})[:]{0,1}(\d{0,2})/i,		p: {			year: 3,			month: 1,			day: 2,			hour: 4,			min: 5,			sec: 6		}	}, {		exp: /(\d{4}).(\d{1,2}).(\d{1,2})[^0-9]*(\d{0,2})[:]{0,1}(\d{0,2})[:]{0,1}(\d{0,2})/i,		p: {			year: 1,			month: 2,			day: 3,			hour: 4,			min: 5,			sec: 6		}	}];	for (var i = 0; i < patt.length; i++) {		var exp = patt[i].exp,		    m = str.match(exp);		if (!time && m && m.length) {			var year = parseInt(m[patt[i].p.year]),			    month = parseInt(m[patt[i].p.month]) || 0,			    day = parseInt(m[patt[i].p.day]) || 1,			    hour = parseInt(m[patt[i].p.hour]) || 0,			    min = parseInt(m[patt[i].p.min]) || 0,			    sec = parseInt(m[patt[i].p.sec]) || 0;			time = new Date(year, month - 1, day, hour, min, sec, 0);			if (patt[i].p.zoneD && m[patt[i].p.zoneD]) {				var delta = m[patt[i].p.zoneD] == '+' ? -1 : 1,				    hour = parseInt(m[patt[i].p.zoneH]) || 0,				    min = parseInt(m[patt[i].p.zoneM]) || 0,				    zone = delta * (min + hour * 60 + time.getTimezoneOffset()) * 60 * 1000;				time = new Date(time.valueOf() + zone);			}		}	}	return time;};_g.timeCal = function (timestamp, hasfix) {	if (timestamp < 60000) {		return Math.floor(timestamp / 1000) + '秒' + (hasfix ? '前' : '');	} else if (timestamp < 3600000) {		return Math.floor(timestamp / 60000) + '分钟' + (hasfix ? '前' : '');	} else if (timestamp < 86400000) {		return Math.floor(timestamp / 3600000) + '小时' + (hasfix ? '前' : '');	} else if (timestamp < 172800000) {		return '昨天';	} else if (timestamp < 2592000000) {		return Math.floor(timestamp / 86400000) + '天' + (hasfix ? '前' : '');	} else if (timestamp < 31536000000) {		return Math.floor(timestamp / 2592000000) + '月' + (hasfix ? '前' : '');	} else {		return Math.floor(timestamp / 31536000000) + '年' + (hasfix ? '前' : '');	}};_g.timeNow = function () {	var now = new Date();	return now.getTime();};_g.timeDiff = function (data) {	var defaults = {		time: _g.timeNow(),		obj: null,		format: _g.time_format,		format2: _g.time_format2,		range: _g.time_diff_range,		range2: _g.time_diff_range2,		is_init: true	};	$.extend(defaults, data);	if (!defaults.obj) return false;	data = defaults;	return data;};_g.timeDiffInterval = function () {	function theinterval() {		if (!_g.isfocus) return false;		for (var i = 0; i < _g.posttime.length; i++) {			var cur = !_g.posttime[i].is_init ? _g.timeDiff(_g.posttime[i]) : _g.posttime[i],			    now = new Date(),			    diff = now.getTime() - cur.time,			    range = cur.range,			    range2 = cur.range2;			if (diff < range) {				cur.obj.html(diff < 60000 ? '刚刚' : _g.timeCal(diff, true));			} else {				var text = diff > range2 ? cur.format2 : cur.format,				    _c = cur.time;				text = text.replace(/Y/g, _c.getFullYear());				text = text.replace(/M/g, _c.getMonth() + 1 < 10 ? '0' + (_c.getMonth() + 1) : _c.getMonth() + 1);				text = text.replace(/m/g, _c.getMonth() + 1);				text = text.replace(/D/g, _c.getDate() < 10 ? '0' + _c.getDate() : _c.getDate());				text = text.replace(/d/g, _c.getDate());				cur.obj.html(text);			}		}	}	clearInterval(_g.interval.posttime);	theinterval();	_g.interval.posttime = setInterval(function () {		theinterval();	}, 60000);};var _support = {	cache: {},	check: function check(technology) {		technology = technology.toLowerCase().replace(/[ :-_]/g, '');		if (typeof _support.cache[technology] == 'undefined' && _support['_' + technology]) {			_support.cache[technology] = _support['_' + technology]();		}		return _support.cache[technology];	}};function _b(string) {	return _support.check(string);}_support._css3 = function () {	return _huCss.csscheck_full('border-radius');};_support._css3animation = function () {	return _huCss.csscheck_full('animation');};_support._css3transition = function () {	return _huCss.csscheck_full('transition');};_support._css3flex = function () {	return _huCss.csscheck_full('flex');};_support._css33d = function () {	return _huCss.csscheck_full('perspective');};_support._css3d = _support._css33d;_support._css3mediaquery = function () {	return window.matchMedia || window.msMatchMedia ? true : false;};_support._mediaquery = _support._css3mediaquery;_support._css3imageset = function () {	var result = null,	    test = $('<div/>', {		'style': 'background-image:url(' + _g.pathImg + '_g-p.gif' + ')'	});	function _test(prefix) {		test.css('background-image', 'image-set(url(' + _g.pathImg + '_g-p.gif' + ') 1x)');		var r = test.css('background-image');		if (r && r != 'none') return true;		if (prefix) {			test.css('background-image', prefix + 'image-set(url(' + _g.pathImg + '_g-p.gif' + ') 1x)');			r = test.css('background-image');			if (r && r != 'none') return prefix;		}		return false;	}	if (bWebkit) {		result = _test('-webkit-');	} else if (bGecko) {		result = _test('-moz-');	} else if (bIE) {		result = _test('-ms-');	} else {		result = _test();	}	return result;};_support._pluginflash = function () {	var _ = false;	try {		_ = typeof navigator.plugins != "undefined" && typeof navigator.plugins["Shockwave Flash"] == "object" || window.ActiveXObject && new ActiveXObject("ShockwaveFlash.ShockwaveFlash") != false;	} catch (e) {}	return _;};_support._flash = _support._pluginflash;_support._html5attrdownload = function () {	return "download" in document.createElement("a");};_support._html5download = _support._html5attrdownload;_support._html5attributedownload = _support._html5attrdownload;_support._html5videomp4 = function () {	var mp4check = document.createElement('video'),	    _ = false;	if (mp4check.canPlayType && mp4check.canPlayType('video/mp4').replace(/no/, '')) _ = true;	return _;};_support._html5mp4 = _support._html5videomp4;_g.init = function () {	_g.baseSize = _g.get_basesize();	_g.baseMultiper = parseFloat(_g.baseSize / 10);	_g.lastBaseSize = _g.get_basesize();	_g.isfocus = Visibility.state() == 'visible';	_g.everfocus = _g.isfocus;	Visibility.change(function (e, state) {		if (state == 'visible') {			_g.isfocus = true;			if (!_g.everfocus) {				_g.everfocus = true;				_g.last.width = -1;				_g.last.height = -1;				_frame.main.last.width = -1;				_frame.main.last.height = -1;			}			$window.trigger('resized_check._g');			_g.timeDiffInterval();		} else {			_g.isfocus = false;		}	});	$window.on({		'orientationchange': function orientationchange() {			$window.trigger('resize');		},		'huResized': function huResized() {			$window.trigger('resized_check._g');		},		'resized_check._g': function resized_check_g() {			if (!_g.isfocus) return false;			var w = $window,			    _width = w.width(),			    _height = w.height();			_g.baseSize = _g.get_basesize();			_g.orientation = _width >= _height ? 'landscape' : 'portrait';			if (_g.baseSize != _g.lastBaseSize) {				_g.baseMultiper = parseFloat(_g.baseSize / 10);				_g.lastBaseSize = _g.baseSize;				_g.isBaseChange = true;				w.trigger('basechange');			}			if (_width != _g.last.width || _height != _g.last.height) {				var data = {					is_widthchange: _width != _g.last.width,					is_heightchange: _height != _g.last.height				};				w.trigger('resized_before', [data]).trigger('resized', [data]);				_g.isEverResized = true;			}			_g.last.width = _width;			_g.last.height = _height;		},		'basechange': function basechange() {			if (_g.isBaseChange) {				_g.isBaseChange = false;				setTimeout(function () {					$window.trigger('resized_before').trigger('resized', [{						is_basechange: true					}]);				}, _g.animate_duration_delay);			}		},		'load.trigger_resized': function loadTrigger_resized() {			if (!_g.isfocus) return false;			setTimeout(function () {				$window.trigger('resized_before', [{					is_load: true				}]).trigger('resized', [{					is_load: true				}]);			}, _g.readyLock ? _g.animate_duration * 4 + 10 : 0);			_g.isload = true;		},		"hashchange._global": function hashchange_global(e) {			_g.uriHashArr = {};			var _h = (location.hash ? location.hash.split('#')[1] : '').split('&');			for (var i in _h) {				var h = _h[i].split('=');				if (h[0] !== '') _g.uriHashArr[h[0]] = h[1] || false;			}			_g.uriHashInited = true;			if (!_g.uriHash() || _g.uriHash() == '') {				e.preventDefault();				e.stopPropagation();			}		},		'unload.focuswindow': function unloadFocuswindow() {			$(this).focus();		}	});	setTimeout(function () {		$window.trigger('hashchange');	}, _g.animate_duration_delay + 10);	$document.on({		'huScrolled': function huScrolled() {			$window.trigger('scrolled');		}	});	$body.data('preventMouseover', false).on({		'touchstart.preventMouseover': function touchstartPreventMouseover() {			$body.removeClass('hover');			$body_preventMouseover = true;		},		'touchend.preventMouseover': function touchendPreventMouseover() {			setTimeout(function () {				$body_preventMouseover = false;			}, 1);		},		'pointerover': function pointerover(e) {			if (e.originalEvent.pointerType == 'touch') $body.trigger('touchstart.preventMouseover');		},		'pointerleave': function pointerleave(e) {			if (e.originalEvent.pointerType == 'touch') $body.trigger('touchend.preventMouseover');		},		'mouseenter': function mouseenter() {			if (!$body_preventMouseover) {				$body.addClass('hover');			} else {					$body_preventMouseover = false;				}		},		'mouseleave': function mouseleave() {			$body.removeClass('hover');		}	});	if (top != self) {		try {			if (self.location.host != top.location.host) top.location.replace(self.location.href);			if (self.location.host == top.location.host) {				$body.addClass('only-content');				_g.changeTheme(top._g.getTheme());			}			$html.css('font-size', top.$html.css('font-size'));			$window.on({				'resized.iframe_resize': function resizedIframe_resize() {					$html.css('font-size', top.$html.css('font-size'));				}			});		} catch (e) {}	}	_hotkey.init();	_frame.init_all();	_p.init_document_ready();	$html.addClass('ready');	$window.trigger('resize', [{ isInitial: true }]);	_g.readyLock = true;	setTimeout(function () {		_g.readyLock = null;	}, _g.animate_duration * 4 + 10);	try {		window.external.msSiteModeClearBadge();	} catch (e) {}	_g.isinit = true;	return false;};function addHandler(object, event, handler) {	if (typeof object.addEventListener != 'undefined') object.addEventListener(event, handler, false);else if (typeof object.attachEvent != 'undefined') object.attachEvent('on' + event, handler);else throw 'Incompatible browser';}jQuery.cookie = function (name, value, options) {	if (typeof value != 'undefined') {		options = options || {};		if (value === null) {			value = '';			options.expires = -1;		}		var expires = '';		if (options.expires && (typeof options.expires == 'number' || options.expires.toUTCString)) {			var date;			if (typeof options.expires == 'number') {				date = new Date();				date.setTime(date.getTime() + options.expires * 24 * 60 * 60 * 1000);			} else {				date = options.expires;			}			expires = '; expires=' + date.toUTCString();		}		var path = '; path=' + (options.path ? options.path : '/');		var domain = options.domain ? '; domain=' + options.domain : /(.*)([\.]*)acgdb\.com/.test(location.host) ? '; domain=.acgdb.com' : '';		var secure = options.secure ? '; secure' : '';		document.cookie = [name, '=', encodeURIComponent(value), expires, path, domain, secure].join('');	} else {		var cookieValue = null;		if (document.cookie && document.cookie != '') {			var cookies = document.cookie.split(';');			for (var i = 0; i < cookies.length; i++) {				var cookie = jQuery.trim(cookies[i]);				if (cookie.substring(0, name.length + 1) == name + '=') {					cookieValue = decodeURIComponent(cookie.substring(name.length + 1));					break;				}			}		}		return cookieValue;	}};(function ($) {	$.fn.innerWidth = function () {		return parseFloat(this.css('width'));	};	$.fn.innerHeight = function () {		return parseFloat(this.css('height'));	};})(jQuery);$.fn.serializeObject = function () {	var o = {};	var a = this.serializeArray();	function _mobj(obj, arr, val) {		if (!arr.length) {			return obj;		}		var e = arr[0];		if (typeof obj[e] == 'undefined') {			obj[e] = arr.length > 1 ? {} : val;		} else if (arr.length == 1) {			if (!obj[e].push) {				obj[e] = [obj[e]];			}			obj[e].push(_val(val));		}		arr.shift();		_mobj(obj[e], arr, val);	}	function _val(value) {		if (typeof value == 'number' && value === 0) return value;		return value || '';	}	$.each(a, function () {		if (!/^_tabview\-/.test(this.name)) {			this.value = isNaN(this.value) || !this.value ? this.value : parseFloat(this.value);			if (this.name.indexOf('.') > -1) {				var sep = this.name.split('.');				_mobj(o, sep, this.value);			} else {				if (o[this.name] || o[this.name] === 0) {					if (!o[this.name].push) {						o[this.name] = [o[this.name]];					}					o[this.name].push(_val(this.value));				} else {					o[this.name] = _val(this.value);				}			}		}	});	return o;};_p.initDOM = function (tar) {	tar = tar || (_frame.dom.layout || (_frame.dom.layout || $body));	return tar.initAll();};_p.initAll = _p.initDOM;$.fn.initDOM = function () {	for (var i in _p.el) {		if (_p.el[i].init) _p.el[i].init(this);	}	return this;};$.fn.initAll = $.fn.initDOM;_p.init_document_ready = function () {	if (!_p.is_init_document_ready) {		_p.initDOM($body);		_p.is_init_document_ready = true;	}};_p.getSummary = function () {	var summary = $('#summary').text() || false;	if (!summary && $('head').find('meta[name=keywords]').length) {		summary = $('head').find('meta[name=Description]').attr('content');		summary = summary.substr(0, summary.indexOf(' - ACGDB'));	}	return summary;}, _p.get_tar = function (tar, className, is_startwith) {	tar = tar || (_frame.dom.layout || ($('#layout') || $body));	if (className.substr(0, 1) == '.') className = className.substr(1);	if (tar.hasClass(className)) return tar;	if (is_startwith) return tar.find('[class^="' + className + '"]');	return tar.find('.' + className);};_p.hashlinks = function (tar) {	tar = tar || $body;	tar.find('a[href^=#]').each(function () {		$(this).off('click.hashlink').on({			'click.hashlink': function clickHashlink() {				_g.goto_hash($(this).attr('href'));				return false;			}		});	});};_p.get_lines = function (el, el_lineheight) {	el_lineheight = el_lineheight || el;	return Math.max(1, Math.floor(Math.min(el.innerHeight(), el.height()) / parseFloat(el_lineheight.css('line-height'))));};_p.el.time = {	init: function init(tar) {		var els = _p.get_tar(tar, '.time');		els.each(function () {			var o = $(this),			    str = o.text();			time = str ? _g.time(str) : null;			if (time) {				_g.posttime.push({					time: time,					obj: o				});			}		});		_g.timeDiffInterval();	}};_p.el.timeSec = {	init: function init(tar) {		var els = _p.get_tar(tar, '.time-sec');		els.each(function () {			var o = $(this);			if (o.data('acgdb-time-sec')) return o.data('acgdb-time-sec');			var str = o.text(),			    time = /\s*([0-9]+)/.exec(str);			if (time && time.length > 1) {				time = parseInt(time[1]);				var m = Math.floor(time / 60),				    s = time % 60,				    t = (m < 10 ? '0' : '') + m + ':' + (s < 10 ? '0' : '') + s + "'";				o.text(t);			}			o.data('acgdb-time-sec', time);		});	}};_p.removeEmptyTextNode = function (el) {	el = $(el);	if (!el.length) return false;	el.contents().filter(function () {		return this.nodeType === 3;	}).remove();};Object.defineProperty(Array.prototype, 'mergeFrom', {	enumerable: false,	value: function value(arr2) {		Array.prototype.push.apply(this, arr2 instanceof Array ? arr2 : [arr2]);		return this;	}});Date.prototype.format = function (pattern, set) {	return _g.formatTime(this, pattern, set);};_g.formatTime_string = {	'zh_CN': {		'Midnight': '深夜',		'Sunday': '周日',
		'Monday': '周一',
		'Tuesday': '周二',
		'Wednesday': '周三',
		'Thursday': '周四',
...
_menu.prototype.init = function () {
	...
	this.dom.menu.on({
		'transitionend.menu_hide': function transitionendMenu_hide(e) {
...
_frame.modal.init = function () {
	...
	_frame.modal.dom.container = $('<div class="container modal" />').on({
		'transitionend.modal_hide': function transitionendModal_hide(e) {
...
_p.tip = {
	...
	init_global: function init_global() {
		...
		_p.tip.dom = $('<div id="tip"/>').on('transitionend', function (e) {
...
var _ga = {
	counter: function counter(path, title, screenName) {
		...
		_frame.dom.hiddenIframe[0].contentWindow.location.replace('/ga.html' + path + (title ? '&title=' + encodeURIComponent(title) : ''));
	}
};
...
_frame.app_main = {
	...
	load_page_func: function load_page_func(page, options) {
		...
		if (!options.callback_modeSelection_select) {
			if (_frame.dom.layout.hasClass('ready')) _frame.app_main.change_bgimg();
	...
	only_bg_on: function only_bg_on() {
		...
		if (!_frame.dom.bg_controls) {
			_frame.dom.bg_controls = $('<div class="bg_controls"/>').on('transitionend.only_bg_off', function (e) {
				if (e.currentTarget == e.target && e.originalEvent.propertyName == 'top' && _frame.dom.layout.hasClass('only_bg') && $(this).offset().top >= $body.height()) {
	...
	init: function init() {
		...
		_frame.dom.logo = $('<button class="logo"/>').on({
			'animationend, webkitAnimationEnd': function animationendWebkitAnimationEnd(e) {
		...
		promise_chain.then(function () {
			...
			var link_page = function link_page(e) {
				...
				var el = $(this),
				    ...
				    href_parts = el.attr('href').split('/').filter(function (c) {
					return c;
				});
			...
			$body.on('click.pagechange', 'a[href^="?page="]', link_page).on('click.pagechange', 'a[href^="?infos="]', link_infos).on('click.pagechange', 'a[href^="/"]', link_default);
...
_tmpl.improvement_inEquipmentInfos = function (equipment, returnHTML) {
	...
	data.forEach(function (improvement) {
		...
		html += '<span class="unit improvement improvement-details">' + '<b>' + (upgrade_to ? '<span class="indicator true">可升级为</span>' + '<a style="background-image:url(/!/assets/images/itemicon/' + upgrade_to.getIconId() + '.png)"' + ' href="?infos=equipment&id=' + upgrade_to['id'] + '"' + ' data-infos="[[EQUIPMENT::' + upgrade_to['id'] + ']]"' + ' data-tip="[[EQUIPMENT::' + upgrade_to['id'] + ']]"' + '">' + upgrade_to.getName(true) + '</a>' + (improvement['upgrade'][1] ? '<i>+' + improvement['upgrade'][1] + '</i>' : '') : '<span class="indicator false">不可升级</span>') + '</b>' + requirements + _tmpl.improvement__resource(improvement, upgrade_to ? true : false) + '</span>';
	}, this);
...
_tmpl.improvement__resource = function (improvement, upgradable) {
	...
	for (var i = 1; i < 4; i++) {
		...
		resource[i] = '<span>' + '<em>' + title + '</em>' + (i == 3 && !upgradable ? '<i class="no">-</i>' : '<i class="dev_mat">' + getValue(improvement['resource'][i][0]) + '<i>(' + getValue(improvement['resource'][i][1]) + ')</i>' + '</i>' + '<i class="imp_mat">' + getValue(improvement['resource'][i][2]) + '<i>(' + getValue(improvement['resource'][i][3]) + ')</i>' + '</i>' + (improvement['resource'][i][4] ? '<a class="equipment"' + ' style="background-image:url(/!/assets/images/itemicon/' + _g.data.items[improvement['resource'][i][4]].getIconId() + '.png)"' + ' href="?infos=equipment&id=' + improvement['resource'][i][4] + '"' + ' data-infos="[[EQUIPMENT::' + improvement['resource'][i][4] + ']]"' + ' data-tip="[[EQUIPMENT::' + improvement['resource'][i][4] + ']]"' + '>' + _g.data.items[improvement['resource'][i][4]].getName(true) + '<i>x' + getValue(improvement['resource'][i][5]) + '</i>' + '</a>' : '')) + '</span>';
	}
...
_frame.app_main.page['arsenal'].init = function (page) {
	...
	this.elMain = $('<div class="main"/>').append(this.init_weekday()).append(this.init_all()).appendTo(page);
...
_frame.app_main.page['arsenal'].init_weekday = function () {
	...
	$('<input/>', {
		...
		Lockr.set('arsenal_weekday-showmeterials', $(this).prop('checked') ? 1 : 0);
	}).prependTo(body);
...
_frame.infos = {
	...
	getContent: function getContent(type, id) {
		...
		if (id == '__NEW__') return _p.initDOM(_frame.infos['__' + type](id).addClass('infosbody'));
	...
	show_func: function show_func(type, id, doNotPushHistory, infosHistoryIndex) {
		...
		if (!_frame.infos.dom) {
			...
			_frame.dom.btnHistoryBack.on({
				'transitionend.infos_hide': function transitionendInfos_hide(e) {
		...
		contentDOM.on('transitionend.hide', function (e) {
	...
	hide: function hide() {
		...
		if (this.lastCurrentPage) {
			if (_frame.dom.navs[this.lastCurrentPage]) _frame.dom.navs[this.lastCurrentPage].addClass('on');
...
_frame.infos.__ship = function (id) {
	...
	function _val(val, show_zero) {
		if (!show_zero && (val == 0 || val == '0')) return '<small class="zero">-</small>';
		if (val == -1 || val == '-1') return '<small class="zero">?</small>';
		return val;
	}
	function _add_stat(name, title, tar) {
		...
		$('<span/>').html('<small class="stat-' + name + '">' + title + '</small>' + '<em' + (val150 ? ' class="lvl99"' : '') + '>' + val99 + '</em>' + (val150 ? '<em class="lvl150">' + val150 + '</em>' : '')).appendTo(tar);
	}
	...
	var lvlRadio99_id = id + '_stat_lv_99',
	    ...
	    stats = $('<div class="stats"/>').html('<div class="title">' + '<h4 data-content="基础属性">基础属性</h4>' + '<span>' + '<label for="' + lvlRadio99_id + '" class="lvl99">99</label>' + '<label for="' + lvlRadio150_id + '" class="lvl150">150</label>' + '</span>' + '</div>').prepend($('<input type="radio" name="ship_infos_lvl_' + id + '" id="' + lvlRadio99_id + '" value="99"/>').prop('checked', curLvl == 99).on('change', function () {
	...
	while (i < 4) {
		...
		if (typeof d['slot'][i] == 'undefined') {
			...
			var item_data = _g.data.items[d['equip'][i]],
			    item_icon = 'assets/images/itemicon/' + item_data.getIconId() + '.png';
			equip.attr({
			...
			name.html(item_data.getName(true));
			slot.html(d['slot'][i]);
			icon.css('background-image', 'url(' + item_icon + ')');
		}
		i++;
	}
	...
	if (d['additional_item_types'] && d['additional_item_types'].length) {
		...
		d['additional_item_types'].forEach(function (currentValue) {
			...
			_additional_equipment_types.append($('<span/>').html(_d['name'][_g.lang]).css({
				'background-image': 'url(assets/images/itemicon/' + _d['icon'] + '.png' + ')'
			}));
		});
	}
	...
	if (seriesData) {
		(function () {
			var check_append = function check_append(file) {
				try {
					var stat = node.fs.lstatSync(file);
					if (stat && stat.isFile()) {
						index++;
						$('<input type="radio" name="ship_' + d['id'] + '_illustrations" value="' + index + '"/>').prop('checked', index == 1).insertBefore(illusts_container);
						$('<span class="container"/>').html('<img src="' + file + '" data-filename="' + ship_name + ' - ' + index + '.webp"/>').appendTo(illusts_container);
					}
				} catch (e) {}
			};
			...
			seriesData.forEach(function (currentValue, i) {
				...
				}).addClass(currentValue['id'] == d['id'] ? 'on' : '').addClass(remodel_blueprint ? 'mod-blueprint' : '').addClass(remodel_catapult ? 'mod-catapult' : '').html('<i><img src="' + _g.path.pics.ships + '/' + currentValue['id'] + '/0.webp"/></i>' + (remodel_lvl ? '<strong>' + remodel_lvl + '</strong>' : '')));
...
_frame.infos.__equipment = function (id) {
	...
	$('<div class="title"/>').html('<h2 data-content="' + d.getName() + '">' + d.getName() + '</h2>' + '<small>' + '<span data-tip="图鉴编号">No.' + d['id'] + '</span>' + (d['type'] ? d.getType() + TablelistEquipments.gen_helper_equipable_on(d['type']) : '') + '</small>' + '<small>' + '<small class="indicator ' + (d['craftable'] ? 'true' : 'false') + '">' + (d['craftable'] ? '可开发' : '不可开发') + '</small>' + '<small class="indicator ' + (d['improvable'] ? 'true' : 'false') + '">' + (d['improvable'] ? '可改修' : '不可改修') + '</small>' + '<small class="indicator ' + (d['upgrade_to'] && d['upgrade_to'].push && d['upgrade_to'].length ? 'true' : 'false') + '">' + (d['upgrade_to'] && d['upgrade_to'].push && d['upgrade_to'].length ? '可升级' : '不可升级') + '</small>' + '</small>').appendTo(dom);
...
if (typeof _p.tip != 'undefined') {
	...
	_p.tip.content_ship = function (d) {
		var ship_name = d.getName(_g.joint),
		    html = '<h3 class="shipinfo">' + '<img src="' + _g.path.pics.ships + '/' + d['id'] + '/0.webp"/>' + '<strong data-content="' + ship_name + '">' + ship_name + '</strong>' + (d['type'] ? '<small>' + _g['data']['ship_types'][d['type']]['full_zh'] + '</span>' : '') + '</h3>';
	...
	_createClass(Tablelist, [{
		...
		value: function mark_high(cacheSortData) {
			...
			rows.eq(0).find('td[data-value]').each((function (index, element) {
				...
				if (cacheSortData) sort_data_by_stat[stat] = sort;else delete sort_data_by_stat[stat];
			}).bind(this));
...
var TablelistEntities = (function (_Tablelist) {
	...
	_createClass(TablelistEntities, [{
		key: 'append_item_cv',
		value: function append_item_cv(entity) {
			return _tmpl.link_entity(entity, null, false, entity.relation.cv.length).addClass('unit cv');
		}
	}, {
		key: 'append_item_illustrator',
		value: function append_item_illustrator(entity) {
			return $('<a/>', {
		...
		key: 'append_items',
		value: function append_items(title, arr, callback_append_item) {
		...
		key: 'init_new',
		value: function init_new() {
			var listCV = [],
			    listIllustrator = [];

			for (var _i11 in _g.data.entities) {
				var entity = _g.data.entities[_i11];
...
var TablelistEquipments = (function (_Tablelist2) {
	...
	_createClass(TablelistEquipments, [{
		key: 'append_item',
		value: function append_item(equipment_data, collection_id) {
			var tr = $('<tr/>', {
				'class': 'row',
			...
			function _val(val, show_zero) {
				if (!show_zero && (val == 0 || val === '0' || val === '')) return '<small class="zero">-</small>';
			...
			this.columns.forEach(function (currentValue) {
				switch (currentValue[1]) {
					...
					case 'improvable':
						...
						break;
					default:
						$('<td data-stat="' + currentValue[1] + '" data-value="' + equipment_data['stat'][currentValue[1]] + '"/>').addClass(equipment_data['stat'][currentValue[1]] < 0 ? 'negative' : '').html(_val(equipment_data['stat'][currentValue[1]])).appendTo(tr);
						break;
				}
			});
		...
		value: function append_all_items() {
		...
		value: function parse_all_items() {
			...
			this.dom.tbody.children('tr.typetitle,tr.row').each((function (index, tr) {
				...
				if (tr.hasClass('typetitle')) {
					...
					(function () {
						var etype = tr.attr('data-equipmenttype'),
			...
			if (Lockr.get('fleetlist-option-groupbytheme')) {
				(function () {
					...
					for (var _i12 in sorted) {
						...
						sorted[_i12].forEach((function (index) {
...
var TablelistShips = (function (_Tablelist4) {
	...
	function TablelistShips(container, options) {
		...
		this.columns = ['  ', ['火力', 'fire'], ['雷装', 'torpedo'], ['夜战', 'nightpower'], ['对空', 'aa'], ['对潜', 'asw'], ['耐久', 'hp'], ['装甲', 'armor'], ['回避', 'evasion'], ['搭载', 'carry'], ['航速', 'speed'], ['射程', 'range'], ['索敌', 'los'], ['运', 'luck'], ['油耗', 'consum_fuel'], ['弹耗', 'consum_ammo']];
	...
	_createClass(TablelistShips, [{
		key: 'append_item',
		value: function append_item(ship_data, header_index) {
			...
			this.columns.forEach(function (currentValue, i) {
				switch (currentValue[1]) {
					case ' ':
						$('<th/>').html('<img src="/!/pics/ships/' + ship_data['id'] + '/0.webp" contextmenu="disabled"/>' + '<strong>' + name + '</strong>' + '<em></em>').prepend(checkbox).appendTo(tr);
						break;
					case 'nightpower':
						...
						break;
					case 'asw':
						$('<td data-stat="asw" />').attr('data-value', ship_data['stat']['asw_max']).html(_val(ship_data['stat']['asw_max'], /^(5|8|9|12|24)$/.test(ship_data['type']))).appendTo(tr);
						break;
					case 'hp':
						$('<td data-stat="hp" data-value="' + ship_data['stat']['hp'] + '"/>').html(_val(ship_data['stat']['hp'])).appendTo(tr);
						break;
					case 'carry':
						$('<td data-stat="carry" data-value="' + ship_data['stat']['carry'] + '"/>').html(_val(ship_data['stat']['carry'])).appendTo(tr);
						break;
					case 'speed':
						$('<td data-stat="speed" data-value="' + ship_data['stat']['speed'] + '"/>').html(_g.getStatSpeed(ship_data['stat']['speed'])).appendTo(tr);
						break;
					case 'range':
						$('<td data-stat="range" data-value="' + ship_data['stat']['range'] + '"/>').html(_g.getStatRange(ship_data['stat']['range'])).appendTo(tr);
						break;
					case 'luck':
						$('<td data-stat="luck" data-value="' + ship_data['stat']['luck'] + '"/>').html(ship_data['stat']['luck'] + '<sup>' + ship_data['stat']['luck_max'] + '</sup>').appendTo(tr);
						break;
					case 'consum_fuel':
						$('<td data-stat="consum_fuel"/>').attr('data-value', ship_data['consum']['fuel']).html(_val(ship_data['consum']['fuel'])).appendTo(tr);
						break;
					case 'consum_ammo':
						$('<td data-stat="consum_ammo"/>').attr('data-value', ship_data['consum']['ammo']).html(_val(ship_data['consum']['ammo'])).appendTo(tr);
						break;
					default:
						$('<td data-stat="' + currentValue[1] + '"/>').attr('data-value', ship_data['stat'][currentValue[1] + '_max']).html(_val(ship_data['stat'][currentValue[1] + '_max'])).appendTo(tr);
						break;
				}
			});
				...
				if (_g.data.ship_id_by_type[i]) {
					if (!j) {
						...
						(function () {
							...
							_this3.last_item = $('<tr class="typetitle" data-trindex="' + _this3.trIndex + '">' + '<th colspan="' + (_this3.columns.length + 1) + '">' + '<label for="' + checkbox_id + '">' + _g.data['ship_type_order'][i]['name']['zh_cn'] + (_g.data['ship_type_order'][i]['name']['zh_cn'] == data_shiptype['full_zh'] ? '<small>[' + data_shiptype['code'] + ']</small>' : '') + '</label></th></tr>').appendTo(_this3.dom.tbody);
							_this3.trIndex++;

							k = 0;

							while (k < _this3.flexgrid_empty_count) {
								_index = _this3.trIndex + _g.data.ship_id_by_type[i].length + k;

								$('<tr class="empty" data-trindex="' + _index + '" data-shipid/>').appendTo(_this3.dom.tbody);
								k++;
							}

							checkbox = $('<input type="checkbox" id="' + checkbox_id + '"/>').prop('disabled', _g.data['ship_type_order'][i]['donotcompare'] ? true : false).on({
								'change': function change() {
							...
							}).data('ships', $()).prependTo(_this3.last_item.find('th'));

							_this3.header_checkbox[i] = checkbox;
						})();
					}
		...
		value: function init_parse() {
			...
			this.dom.filters.find('[name="viewtype"]').each((function (index, $el) {
				$el = $($el);
				if ($el.val() == _config.get('shiplist-viewtype')) $el.prop('checked', true);
			...
			this.dom.tbody.children('tr.typetitle,tr.row').each((function (index, tr) {
				...
				if (tr.hasClass('typetitle')) {
					(function () {
						header_index++;
						_this4.last_item = tr;
						var checkbox = tr.find('input[type="checkbox"]').prop('disabled', _g.data['ship_type_order'][header_index]['donotcompare'] ? true : false).on({
							'change': function change() {
								checkbox.data('ships').filter(':visible').each(function (index, element) {
									$(element).data('checkbox').prop('checked', checkbox.prop('checked')).trigger('change', [true]);
								});
						...
						_this4.header_checkbox[header_index] = checkbox;
					})();
				} else {
					(function () {
						...
						_this4.header_checkbox[title_index].data('ships', _this4.header_checkbox[title_index].data('ships').add(tr));

						tr.data('checkbox', checkbox);

						_this4.checkbox[ship_data['id']] = checkbox;
					})();
				}
			}).bind(this));
