...
function buildTDZAssert(node, file) {
  return t.callExpression(file.addHelper("temporal-ref"), [node, t.stringLiteral(node.name), file.addHelper("temporal-undefined")]);
}
  ...
  ReferencedIdentifier: function ReferencedIdentifier(path, state) {
    ...
    if (status === "maybe") {
      ...
      if (path.parentPath.isUpdateExpression()) {
        ...
        return t.sequenceExpression([assert, node]);
      }
    } else if (status === "outside") {
      return t.throwStatement(t.inherits(t.newExpression(t.identifier("ReferenceError"), [t.stringLiteral(node.name + " is not defined - temporal dead zone")]), node));
    }
  },
