modal.bonuses = (() => ({
    ...
    getFrame: function () {
        ...
        obj.getBonuses().forEach((bonus) => {
            if (typeof bonus.equipments === 'object') bonuses.set.push(bonus);
            else bonuses.single.push(bonus);
        });
    ...
    renderStat(bonus) {
        ...
        _g.stats.forEach((arr) => {
            ...
            r += `<span class="${classNames.join(
                ' '
            )}" data-stat="${stat}">${content}</span>`;
        });
        return r;
    },
            ...
            condition +=
                ...
                classes
                    .map((classId) => {
                        ...
                        const type =
                            _g.data.ship_types[parseInt(d.ship_type_id)];
                        return `${d.name.zh_cn || d.name.ja_jp}级${
                            type.name.zh_cn || type.name.ja_jp
                        }`;
                    })
                    .map((s) => `<span class="item ship-class">${s}</span>`)
                    .join('') +
            ...
            condition +=
                ...
                ids
                    .map((shipId) => {
                        ...
                        return (
                            `<a class="item ship ship-link" href="${_g.getLink(
                                'ships',
                                ship.id
                            )}" data-shipid="${ship.id}">` +
                            `<em class="avatar"><img src="${ship.getPic(
                                0,
                                _g.imgExt
                            )}"/></em>` +
                            `${ship._name}` +
        ...
        return (
            ...
            equipments
                .map((condition) => {
                    ...
                    if (!isNaN(id))
                        return _tmpl.link_equipment(id, 'span', true);
                    return '';
                })
                .join('') +
            `</div>` +
    ...
    renderConditionEquipmentOneOfForSet(bonus) {
        if (Array.isArray(bonus.equipments.hasOneOf))
            return this.renderConditionEquipmentOneOf(
                bonus.equipments.hasOneOf
            );
    ...
    renderBonusSingle: function (bonus) {
            ...
            case 'equipment': {
                ...
                const conditionKeys =
                    typeof bonus.ship === 'object'
                        ? Object.keys(bonus.ship).filter(
                              (key) => key.toLowerCase() !== 'canequip'
                          )
                        : [];
        ...
        if (typeof bonus.bonusCount === 'object') {
            ...
            Object.keys(bonus.bonusArea).forEach((area) => {
                bonusStats +=
                    `<div class="has-extra">` +
